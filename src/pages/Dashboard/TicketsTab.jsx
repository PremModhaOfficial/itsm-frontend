...
import { ChevronUp, ChevronDown, Filter, X, Plus } from 'lucide-react'; // Added Plus for consistency
...
import { Pagination } from './DashboardPagination'; // Ensure this path is correct, previously DashboardPagination
...
export const TicketsTab = ({ loading, setLoading, error, setError }) => {
    ...
    const [filters, setFilters] = useState({
        status: '',
        priority: '',
        assigned_technician_id: '', // Changed to _id to match API
        requester_id: '' // Changed to _id to match API
    });
    ...
    useEffect(() => {
        const fetchFilterOptions = async () => {
            try {
                // Fetch technicians for assigned_technician filter
                const techsResponse = await techniciansApi.getAllTechniciansSimple(); // Assuming this returns { data: [...], total: X } or just [...]
                const techsData = techsResponse.data?.technicians || techsResponse.data || [];
                // Map to {id, name} format for dropdown
                setTechnicianOptions(techsData.map(tech => ({ id: tech.id, name: tech.user?.name || tech.name })));

                // Fetch users for requester filter
                const usersResponse = await usersApi.getAllUsers(); // Assuming this returns { data: [...], total: X } or just [...]
                const usersData = usersResponse.data?.users || usersResponse.data || [];
                // Map to {id, name} format for dropdown
                setRequesterOptions(usersData.map(user => ({ id: user.id, name: user.name })));

            } catch (err) {
                console.error("Failed to fetch filter options:", err);
                // Don't set global error, just log. Filter might still work partially.
            }
        };
        fetchFilterOptions();
    }, []); // Run once on mount
    ...
    return (
        <div className="bg-white shadow-xl rounded-lg p-8 transform transition-all duration-300 hover:scale-[1.005]">
            ...
            {/* Filter Panel */}
            {showFilters && (
                <div className="mb-6 p-6 bg-gray-50 rounded-lg border border-gray-200 shadow-inner"> {/* Enhanced styling */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                className="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 py-2 px-3"
                            >
            ...
            {/* Results Summary */}
            {!loading && !error && (
                <div className="mb-4 text-sm text-gray-600 px-2"> {/* Added horizontal padding */}
